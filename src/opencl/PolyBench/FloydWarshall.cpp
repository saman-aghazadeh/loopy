...
void RunBenchmark (cl_device_id dev,
  ...
  for (unsigned long long dataSize = minDataSize; dataSize <= maxDataSize; dataSize *= 2) {
    ...
    void *M, *M_T, *M_out;
    cl_mem clM, clM_T, clM_out;

    cl_kernel kernel = clCreateKernel (program, kernel_name.c_str(), &err);
    CL_CHECK_ERROR (err);

    M = (void *) malloc (dataSize);
    M_T = (void *) malloc (dataSize);
    M_out = (void *) malloc (dataSize);
    ...
    clM = clCreateBuffer (ctx, CL_MEM_READ_ONLY, dataSize, NULL, &err);
    CL_CHECK_ERROR (err);

    clM_T = clCreateBuffer (ctx, CL_MEM_READ_ONLY, dataSize, NULL, &err);
    CL_CHECK_ERROR (err);

    clM_out = clCreateBuffer (ctx, CL_MEM_READ_WRITE, dataSize, NULL, &err);
    CL_CHECK_ERROR (err);

    err = clEnqueueWriteBuffer (queue, clM, CL_TRUE, 0, dataSize, M, 0, NULL, NULL);
    CL_CHECK_ERROR (err);

    err = clEnqueueWriteBuffer (queue, clM_T, CL_TRUE, 0, dataSize, M_T, 0, NULL, NULL);
    CL_CHECK_ERROR (err);

    err = clEnqueueWriteBuffer (queue, clM_out, CL_TRUE, 0, dataSize, M_out, 0, NULL, NULL);
		CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel, 0, sizeof (cl_mem), (void *) &clM);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel, 1, sizeof (cl_mem), (void *) &clM_T);
    CL_CHECK_ERROR (err);

		err = clSetKernelArg (kernel, 2, sizeof (cl_mem), (void *) &clM_out);
    CL_CHECK_ERROR (err);
    ...
    err = clSetKernelArg (kernel, 3, sizeof (int), &lll);
    CL_CHECK_ERROR (err);
    ...
    CL_BAIL_ON_ERROR (err);
      ...
      err = clEnqueueReadBuffer (queue, clM_out, CL_TRUE, 0, dataSize, M_out, 0, 0, &evRead.CLEvent());
    ...
    clReleaseMemObject (clM_T);
    clReleaseMemObject (clM_out);
