...
void RunBenchmark (cl_device_id dev,
  ...
  for (unsigned long long dataSize = minDataSize; dataSize <= maxDataSize; dataSize *= 2) {
    ...
    clD = clCreateBuffer (ctx, CL_MEM_READ_WRITE, dataSize, NULL, &err);
    CL_CHECK_ERROR (err);

    clTemp = clCreateBuffer (ctx, CL_MEM_READ_WRITE, dataSize, NULL, &err);
    CL_CHECK_ERROR (err);
    ...
    if (dataType == "INT") {
      ...
      err = clSetKernelArg (kernel1, 4, sizeof (int), (void *) &alpha);
      CL_CHECK_ERROR (err);

      err = clSetKernelArg (kernel2, 4, sizeof (int), (void *) &alpha);
      CL_CHECK_ERROR (err);

      err = clSetKernelArg (kernel1, 5, sizeof (int), (void *) &beta);
      CL_CHECK_ERROR (err);

      err = clSetKernelArg (kernel2, 5, sizeof (int), (void *) &beta);
      CL_CHECK_ERROR (err);
      ...
      err = clSetKernelArg (kernel1, 4, sizeof (float), (void *) &alpha);
      CL_CHECK_ERROR (err);

      err = clSetKernelArg (kernel2, 4, sizeof (float), (void *) &alpha);
      CL_CHECK_ERROR (err);

      err = clSetKernelArg (kernel1, 5, sizeof (float), (void *) &beta);
      CL_CHECK_ERROR (err);

      err = clSetKernelArg (kernel2, 5, sizeof (float), (void *) &beta);
      CL_CHECK_ERROR (err);
      ...
      err = clSetKernelArg (kernel1, 4, sizeof (double), (void *) &alpha);
      CL_CHECK_ERROR (err);

      err = clSetKernelArg (kernel2, 4, sizeof (double), (void *) &alpha);
      CL_CHECK_ERROR (err);

      err = clSetKernelArg (kernel1, 5, sizeof (double), (void *) &beta);
      CL_CHECK_ERROR (err);

      err = clSetKernelArg (kernel2, 5, sizeof (double), (void *) &beta);
      CL_CHECK_ERROR (err);
    ...
    if (device_type == "FPGA") {
      if (fpga_op_type == "SINGLE") {
        ...
        lll = sqrt(numIterations);
        if (lll * lll != numIterations) continue;

        if (device_type == "GPU") {
          err = clSetKernelArg (kernel1, 6, sizeof (int), &lll);
          CL_CHECK_ERROR (err);

          err = clSetKernelArg (kernel2, 6, sizeof (int), &lll);
          CL_CHECK_ERROR (err);

          err = clSetKernelArg (kernel1, 7, sizeof (cl_mem), (void *) &clTemp);
          CL_CHECK_ERROR (err);

          err = clSetKernelArg (kernel2, 7, sizeof (cl_mem), (void *) &clTemp);
          CL_CHECK_ERROR (err);
        } else if (device_type == "FPGA"){
          err = clSetKernelArg (kernel1, 6, sizeof (int), &lll);
          CL_CHECK_ERROR (err);
    ...
    if (dataType == "INT") {
      global_work_size[0] = sqrt(dataSize/4);
      global_work_size[1] = sqrt(dataSize/4);
    } else if (dataType == "SINGLE") {
      global_work_size[0] = sqrt(dataSize/4);
      global_work_size[1] = sqrt(dataSize/4);
    } else if (dataType == "DOUBLE") {
      global_work_size[0] = sqrt(dataSize/8);
      global_work_size[1] = sqrt(dataSize/8);
    }
    ...
    CL_BAIL_ON_ERROR (err);
      ...
      if (device_type == "FPGA") {
        ...
        cl_ulong start = evKernel1.SubmitTime();
        cl_ulong end = evKernel2.EndTime();
				totalTime = end - start;
      }
