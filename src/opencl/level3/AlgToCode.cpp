...
struct _algorithm_type tests[] = {
	{"Test1", 1, vector<int>({100}), vector<int>({50}), false, vector<int>({0}), 32, 128, 2, 3},
  {0, 0, vector<int>(), vector<int>(), 0, vector<int>(), 0, 0, 0}
};
...
void generateSingleAlgorithm (ostringstream &oss, struct _algorithm_type &test, const char* varType);
...
void generateSingleCLCode (ostringstream &oss, struct _algorithm_type &temp, struct _cl_info &info, const char *varType);
...
void generateSingleCLMeta (struct _algorithm_type &temp, struct _cl_info &info, const char* varType);
...
void generateCLs () {
  ...
  while ((tests != 0) && (tests[aIdx].name != 0)) {
    ostringstream oss_float;
    ostringstream oss_double;
    struct _cl_info test_cl_info_float;
    struct _cl_info test_cl_info_double;
    struct _algorithm_type temp = tests[aIdx];
		generateSingleCLCode (oss_float, temp, test_cl_info_float, "float");
    generateSingleCLCode (oss_double, temp, test_cl_info_double, "double");
    aIdx++;
  }
...
void generateCLsMetas () {
  ...
  while ((tests != 0) && (tests[aIdx].name != 0)) {
    ...
    struct _cl_info test_cl_info_double;
    struct _algorithm_type temp = tests[aIdx];
		generateSingleCLMeta (temp, test_cl_info_float, "float");
    generateSingleCLMeta (temp, test_cl_info_double, "double");
    cl_metas.push_back (test_cl_info_float);
    cl_metas.push_back (test_cl_info_double);
    aIdx++;
  }

}

void generateSingleCLCode (ostringstream &oss, struct _algorithm_type &test, struct _cl_info &info, const char* varType) {
  ...
  if (test.loopCarriedDataDependency == false) {
  	...
  	if (strcmp (varType, "double")) {
		...
		oss << "__kernel void " << test.name << "(__global " << varType << " *data, __global " << varType << " *rands, int index, int rand_max){" << endl;
    oss << endl;
...
void generateSingleCLMeta (_algorithm_type &test, _cl_info &info, const char* varType) {

  info.name = (char *)test.name;

  memcpy (info.kernel_location, (char *)(string(kernels_folder + "/" + test.name + "-" + varType + ".cl").c_str()),
          strlen((char *)(string(kernels_folder + "/" + test.name + "-" + varType + ".cl").c_str())));
...
void generateAlgorithms ( ) {
  ...
  while ((tests != 0) && (tests[aIdx].name != 0)) {
    ...
    ostringstream oss_double;
    struct _algorithm_type temp = tests[aIdx];
		generateSingleAlgorithm (oss_float, temp, "float");
    generateSingleAlgorithm (oss_double, temp, "double");
    aIdx++;
  }
}
...
void generateSingleAlgorithm (ostringstream &oss, struct _algorithm_type &test, const char* varType) {
  ...
  ofstream codeDump;
 	string dumpFileName = kernels_folder + "/" + test.name + "-" + varType + ".cpp";
	codeDump.open (dumpFileName.c_str());
  ...
  oss << endl;
  oss << varType << " f(" << varType << " x);" << endl;
  oss << varType << " generate_random();" << endl;
  oss << endl;
	oss << "int main () {" << endl;
	oss << endl;
  ...
  insertTab (oss, 1); oss << "int L = " << test.loopsLengths[0] <<
	...
	insertTab (oss, 1); oss << varType << "** temp;" << endl;
  insertTab (oss, 1); oss << "temp = new " << varType << "*[D];" << endl;
	insertTab (oss, 1); oss << "for (int j = 0; j < D; j++) {" << endl;
	insertTab (oss, 2); oss << "temp[j] = new " << varType << "[L];" << endl;
  ...
  insertTab (oss, 1); oss << "}";
	oss << endl;
  insertTab (oss, 1); oss << varType << " final = temp[0][0] + temp[" << test.loopsDepth[0] - 1 << "][" << test.loopsLengths[0] - 1 << "];" << endl;
  ...
  oss << endl;
	...
	oss << varType << " f (" << varType << " x) {" << endl;
  oss << endl;
  insertTab (oss, 1); oss << "return x * ((" << varType << ")rand()/(" << varType << ")(RAND_MAX/2));" << endl;
  oss << endl;
  oss << "}" << endl;
	oss << endl;
  oss << varType << " generate_random() {" << endl;
  oss << endl;
  insertTab (oss, 1); oss << "return (" << varType << ")rand()/(" << varType << ")(RAND_MAX/2);" << endl;
  oss << endl;
	oss << "}" << endl;
...
void execution (cl_device_id id,
  ...
  while ((tests != 0) && (tests[aIdx].name != 0)) {
    ...
    int halfNumFloatsMax = alg.halfBufSizeMax * 1024;
    ...
    err = clSetKernelArg (kernel, 0, sizeof (cl_mem), (void *)&hostMem_data);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel, 1, sizeof (cl_mem), (void *)&hostMem_rands);
    CL_CHECK_ERROR (err);
    ...
    for (int halfNumFloats = alg.halfBufSizeMin * 1024;
      ...
      for (int wsBegin = alg.localWorkSizeMin; wsBegin <= alg.localWorkSizeMax; wsBegin *= alg.localWorkSizeStride) {
        ...
        for (int pas = 0; pas < npasses; ++pas) {
          ...
          resultDB.AddResult (string(alg.name) + string("-lws") + string (lwsString), sizeStr, "GFLOPS", gflop);
