  ...
  */
  ...
  for (int workGroupSize = 256; workGroupSize <= 256; workGroupSize *= 2) {
	...
	*/
	...
	int unrollFactor = 0;
  int unrolls[] = {1, 4, 8, 12, 16, 20, 24, 28, 32};
	for (int unrollIndex = 0; unrollIndex < 9; unrollIndex++) {
    	...
    	for (int memAllocationPerWorkItem = 2;
      	...
      	for (int loopLength = 65536; loopLength <= 1048576; loopLength *= 2) {
              ...
              .createFor	(8, false, loopLength, "temp1 += temp1 * MF", 1, false, 2, 32)
             	...
             	.endKernelFunction ()
             	.verbose ()
            	.writeToFile (string("/home/user/sbiookag/Algs/unfolding-test/unroll") + to_string(unrollFactor) +  string(".cl"));
