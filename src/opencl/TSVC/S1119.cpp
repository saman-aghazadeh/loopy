...
void RunBenchmark (cl_device_id dev,
	...
	for (unsigned long long dataSize = minDataSize; dataSize <= maxDataSize; dataSize *= 2) {
		...
		for (int llly = 256; llly < max/4; llly *= 2) {
			...
			clAA = clCreateBuffer (ctx, CL_MEM_READ_ONLY, dataSize, NULL, &err);
			CL_CHECK_ERROR (err);

			clBB = clCreateBuffer (ctx, CL_MEM_READ_ONLY, dataSize, NULL, &err);
			CL_CHECK_ERROR (err);

			clCC = clCreateBuffer (ctx, CL_MEM_WRITE_ONLY, dataSize, NULL, &err);
			CL_CHECK_ERROR (err);
			...
			err = clEnqueueWriteBuffer (queue, clCC, CL_TRUE, 0, dataSize, CC, 0, NULL, NULL);
			CL_CHECK_ERROR (err);
			...
			err = clSetKernelArg (kernel, 2, sizeof (cl_mem), (void *) &clCC);
			CL_CHECK_ERROR (err);
			...
			err = clSetKernelArg (kernel, 3, sizeof (int), &numIterationsX);
			CL_CHECK_ERROR (err);

			if (device_type == "FPGA") {
				if (fpga_op_type == "SINGLE") {
					err = clSetKernelArg (kernel, 4, sizeof (int), &numIterationsY);
					CL_CHECK_ERROR (err);
				}
			}
			...
			for (int iter = 0; iter < passes; iter++) {
				...
				if (dataType == "INT")
					resultDB.AddResult ("KernelINT" /*+ toString(dataSize) + "KiB"*/,
							toString(dataSize)+"-lllX(Dep)"+toString(lllx)+"-lllY(Par)"+toString(llly), "Bytes", evKernel.SubmitEndRuntime());
				else if (dataType == "SINGLE")
				...
				err = clEnqueueReadBuffer (queue, clCC, CL_TRUE, 0, dataSize, CC, 0, 0, &evRead.CLEvent());
