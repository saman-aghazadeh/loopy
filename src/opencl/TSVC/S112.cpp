...
void RunBenchmark (cl_device_id dev,
  ...
  for (unsigned long long dataSize = minDataSize; dataSize <= maxDataSize; dataSize *= 2) {
		...
		int modifiedDataSize = (dataSize / localX) * (localX + 1);

		cout << "[INFO] data size is " << modifiedDataSize << endl;
    ...
    A = (void *) malloc (modifiedDataSize);
    B = (void *) malloc (modifiedDataSize);

    clA = clCreateBuffer (ctx, CL_MEM_READ_WRITE, modifiedDataSize, NULL, &err);
    CL_CHECK_ERROR (err);

    clB = clCreateBuffer (ctx, CL_MEM_READ_WRITE, modifiedDataSize, NULL, &err);
    CL_CHECK_ERROR (err);

    err = clEnqueueWriteBuffer (queue, clA, CL_TRUE, 0, modifiedDataSize, A, 0, NULL, NULL);
    CL_CHECK_ERROR (err);

    err = clEnqueueWriteBuffer (queue, clB, CL_TRUE, 0, modifiedDataSize, B, 0, NULL, NULL);
    CL_CHECK_ERROR (err);
