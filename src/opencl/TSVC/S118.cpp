...
void RunBenchmark (cl_device_id dev,
  ...
  for (unsigned long long dataSize = minDataSize; dataSize <= maxDataSize; dataSize *= 2) {
    ...
    BB = (void *) malloc (dataSize * dataSize);

    clA = clCreateBuffer (ctx, CL_MEM_READ_WRITE, dataSize, NULL, &err);
    CL_CHECK_ERROR (err);

    clBB = clCreateBuffer (ctx, CL_MEM_READ_ONLY, dataSize * dataSize, NULL, &err);
    CL_CHECK_ERROR (err);

    err = clEnqueueWriteBuffer (queue, clA, CL_TRUE, 0, dataSize, A, 0, NULL, NULL);
    CL_CHECK_ERROR (err);

    err = clEnqueueWriteBuffer (queue, clBB, CL_TRUE, 0, dataSize * dataSize, BB, 0, NULL, NULL);
    CL_CHECK_ERROR (err);
    ...
    CL_CHECK_ERROR (err);
      ...
      if (fpga_op_type == "SINGLE") {
        ...
        else if (dataType == "DOUBLE") numIterations = dataType / 8;
