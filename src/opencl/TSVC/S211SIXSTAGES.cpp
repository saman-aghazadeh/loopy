...
void addBenchmarkSpecOptions (OptionParser &op) {
  ...
  op.addOption ("kern3_name", OPT_STRING, "", "name of the kernel 3 function");
  op.addOption ("kern4_name", OPT_STRING, "", "name of the kernel 4 function");
  op.addOption ("kern5_name", OPT_STRING, "", "name of the kernel 5 function");
  op.addOption ("kern6_name", OPT_STRING, "", "name of the kernel 6 function");
...
void RunBenchmark (cl_device_id dev,
  ...
  cl_int status;
	cl_command_queue second_queue, third_queue, fourth_queue, fifth_queue, sixth_queue;

  second_queue = clCreateCommandQueue (ctx, dev, CL_QUEUE_PROFILING_ENABLE, &status);
	CL_CHECK_ERROR (status);

	third_queue = clCreateCommandQueue (ctx, dev, CL_QUEUE_PROFILING_ENABLE, &status);
  CL_CHECK_ERROR (status);

  fourth_queue = clCreateCommandQueue (ctx, dev, CL_QUEUE_PROFILING_ENABLE, &status);
  CL_CHECK_ERROR (status);

  fifth_queue = clCreateCommandQueue (ctx, dev, CL_QUEUE_PROFILING_ENABLE, &status);
  CL_CHECK_ERROR (status);

  sixth_queue = clCreateCommandQueue (ctx, dev, CL_QUEUE_PROFILING_ENABLE, &status);
  CL_CHECK_ERROR (status);
  ...
  string kernel3_name = op.getOptionString("kern3_name");
  string kernel4_name = op.getOptionString("kern4_name");
  string kernel5_name = op.getOptionString("kern5_name");
  string kernel6_name = op.getOptionString("kern6_name");
  ...
  cout << "[INFO] Third kernel is " << kernel3_name << endl;
  cout << "[INFO] Fourth kernel is " << kernel4_name << endl;
  ...
  for (unsigned long long dataSize = minDataSize; dataSize <= maxDataSize; dataSize *= 2) {
    ...
    CL_CHECK_ERROR (err);
		...
		cl_kernel kernel3 = clCreateKernel (program, kernel3_name.c_str(), &err);
    CL_CHECK_ERROR (err);

    cl_kernel kernel4 = clCreateKernel (program, kernel4_name.c_str(), &err);
    CL_CHECK_ERROR (err);

    cl_kernel kernel5 = clCreateKernel (program, kernel5_name.c_str(), &err);
    CL_CHECK_ERROR (err);

    cl_kernel kernel6 = clCreateKernel (program, kernel6_name.c_str(), &err);
    CL_CHECK_ERROR (err);
    ...
    err = clSetKernelArg (kernel3, 0, sizeof (cl_mem), (void *) &clA);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel3, 1, sizeof (cl_mem), (void *) &clB);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel3, 2, sizeof (cl_mem), (void *) &clBPrime);
    CL_CHECK_ERROR (err);

		err = clSetKernelArg (kernel3, 3, sizeof (cl_mem), (void *) &clC);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel3, 4, sizeof (cl_mem), (void *) &clD);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel3, 5, sizeof (cl_mem), (void *) &clE);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel4, 0, sizeof (cl_mem), (void *) &clA);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel4, 1, sizeof (cl_mem), (void *) &clB);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel4, 2, sizeof (cl_mem), (void *) &clBPrime);
    CL_CHECK_ERROR (err);

		err = clSetKernelArg (kernel4, 3, sizeof (cl_mem), (void *) &clC);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel4, 4, sizeof (cl_mem), (void *) &clD);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel4, 5, sizeof (cl_mem), (void *) &clE);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel5, 0, sizeof (cl_mem), (void *) &clA);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel5, 1, sizeof (cl_mem), (void *) &clB);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel5, 2, sizeof (cl_mem), (void *) &clBPrime);
    CL_CHECK_ERROR (err);

		err = clSetKernelArg (kernel5, 3, sizeof (cl_mem), (void *) &clC);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel5, 4, sizeof (cl_mem), (void *) &clD);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel5, 5, sizeof (cl_mem), (void *) &clE);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel6, 0, sizeof (cl_mem), (void *) &clA);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel6, 1, sizeof (cl_mem), (void *) &clB);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel6, 2, sizeof (cl_mem), (void *) &clBPrime);
    CL_CHECK_ERROR (err);

		err = clSetKernelArg (kernel6, 3, sizeof (cl_mem), (void *) &clC);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel6, 4, sizeof (cl_mem), (void *) &clD);
    CL_CHECK_ERROR (err);

    err = clSetKernelArg (kernel6, 5, sizeof (cl_mem), (void *) &clE);
    CL_CHECK_ERROR (err);
    ...
    if (device_type == "FPGA") {
      if (fpga_op_type == "SINGLE") {
        ...
        err = clSetKernelArg (kernel3, 6, sizeof (int), &numIterations);
        CL_CHECK_ERROR (err);

        err = clSetKernelArg (kernel4, 6, sizeof (int), &numIterations);
        CL_CHECK_ERROR (err);

        err = clSetKernelArg (kernel5, 6, sizeof (int), &numIterations);
        CL_CHECK_ERROR (err);

        err = clSetKernelArg (kernel6, 6, sizeof (int), &numIterations);
        CL_CHECK_ERROR (err);
    ...
    Event evKernel5 ("KernelEvent4");
    Event evKernel6 ("KernelEvent4");
    ...
    if (device_type == "FPGA" && fpga_op_type == "SINGLE") {
      if (use_channel == 0) {
        ...
        err = clEnqueueTask (queue, kernel3, 2, &evKernel2.CLEvent(), &evKernel3.CLEvent());
        err = clEnqueueTask (queue, kernel4, 3, &evKernel3.CLEvent(), &evKernel4.CLEvent());
        err = clEnqueueTask (queue, kernel5, 3, &evKernel4.CLEvent(), &evKernel5.CLEvent());
        err = clEnqueueTask (queue, kernel6, 3, &evKernel5.CLEvent(), &evKernel6.CLEvent());
      } else if (use_channel == 1) {
        err = clEnqueueTask (sixth_queue, kernel6, 0, NULL, &evKernel6.CLEvent());
        err = clEnqueueTask (fifth_queue, kernel5, 0, NULL, &evKernel5.CLEvent());
        err = clEnqueueTask (fourth_queue, kernel4, 0, NULL, &evKernel4.CLEvent());
        err = clEnqueueTask (third_queue, kernel3, 0, NULL, &evKernel3.CLEvent());
      ...
      err = clEnqueueNDRangeKernel (queue, kernel3, 1,
                                    NULL, global_work_size, local_work_size,
                                    1, &evKernel2.CLEvent(), &evKernel3.CLEvent());
			err = clEnqueueNDRangeKernel (queue, kernel4, 1,
                                    NULL, global_work_size, local_work_size,
                                    1, &evKernel3.CLEvent(), &evKernel4.CLEvent());
      err = clEnqueueNDRangeKernel (queue, kernel5, 1,
                                    NULL, global_work_size, local_work_size,
                                    1, &evKernel4.CLEvent(), &evKernel5.CLEvent());
     	err = clEnqueueNDRangeKernel (queue, kernel6, 1,
                                    NULL, global_work_size, local_work_size,
                                    1, &evKernel5.CLEvent(), &evKernel6.CLEvent());
    ...
    CL_BAIL_ON_ERROR (err);
      ...
      if (device_type == "FPGA" && fpga_op_type == "SINGLE") {
        if (use_channel == false) {
          err = clEnqueueTask (queue, kernel1, 0, NULL, &evKernel1.CLEvent());
          err = clEnqueueTask (queue, kernel2, 1, &evKernel1.CLEvent(), &evKernel2.CLEvent());
          err = clEnqueueTask (queue, kernel3, 1, &evKernel2.CLEvent(), &evKernel3.CLEvent());
          err = clEnqueueTask (queue, kernel4, 1, &evKernel3.CLEvent(), &evKernel4.CLEvent());
          err = clEnqueueTask (queue, kernel5, 1, &evKernel4.CLEvent(), &evKernel5.CLEvent());
          err = clEnqueueTask (queue, kernel6, 1, &evKernel5.CLEvent(), &evKernel6.CLEvent());
      ...
      evKernel5.FillTimingInfo();
      evKernel6.FillTimingInfo();
        ...
        cl_ulong start3 = evKernel3.SubmitTime();
				cl_ulong start4 = evKernel4.SubmitTime();
				cl_ulong start5 = evKernel5.SubmitTime();
				cl_ulong start6 = evKernel6.SubmitTime();
        ...
        cl_ulong end5 = evKernel5.EndTime();
        cl_ulong end6 = evKernel6.EndTime();

        cl_ulong start = (start1 > start2) ? start2 : start1;
      	start = (start < start3) ? start : start3;
        start = (start < start4) ? start : start4;
        start = (start < start5) ? start : start5;
        start = (start < start6) ? start : start6;
        cl_ulong end = (end1 > end2) ? end1 : end2;
        end = (end < end3) ? end3 : end;
        end = (end < end4) ? end4 : end;
        end = (end < end5) ? end5 : end;
        end = (end < end6) ? end6 : end;

				totalTime = end - start;

      } else {
        totalTime = evKernel1.SubmitEndRuntime() + evKernel2.SubmitEndRuntime()
          + evKernel3.SubmitEndRuntime() + evKernel4.SubmitEndRuntime()
          + evKernel5.SubmitEndRuntime() + evKernel6.SubmitEndRuntime();
      }
      if (dataType == "INT")
