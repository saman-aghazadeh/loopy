...
void RunBenchmark (cl_device_id dev,
  ...
  for (unsigned long long dataSize = minDataSize; dataSize <= maxDataSize; dataSize *= 4) {
    ...
    */
    ...
    CL_BAIL_ON_ERROR (err);
      ...
      // Initialization of all the events
      for (int i = 0; i < 2*(sizeX-1); i++) {
        Event* event = new Event("Event" + toString(i));
        allEvents.push_back(event);
      }

			cout << "Enqueuing write buffer" << endl;
      ...
      if (device_type == "FPGA" && fpga_op_type == "SINGLE") {
        ...
        int startY = 0;
          ...
          err = clSetKernelArg (kernel, 2, sizeof (int), (void *) &startX);
          CL_CHECK_ERROR (err);
          err = clSetKernelArg (kernel, 3, sizeof (int), (void *) &startY);
          CL_CHECK_ERROR (err);

          if (i == 0) {
            err = clEnqueueNDRangeKernel (queue, kernel, 2,
                                          NULL, global_work_size, local_work_size,
                                          0, NULL, &((allEvents.at(i))->CLEvent()));
            CL_CHECK_ERROR (err);
         } else {
            err = clEnqueueNDRangeKernel (queue, kernel, 2,
                                          NULL, global_work_size, local_work_size,
                                          1, &((allEvents.at(i-1))->CLEvent()), &((allEvents.at(i))->CLEvent()));
         }
      ...
      if (dataType == "INT")
        ...
        resultDB.AddResult ("KernelSINGLE",
                            toString(dataSize) + "-" + toString(sizeX) + "-" + toString(sizeY), "Bytes", count);
      else if (dataType == "DOUBLE")
