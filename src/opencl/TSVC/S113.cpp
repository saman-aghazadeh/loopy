...
void RunBenchmark (cl_device_id dev,
  ...
  for (unsigned long long dataSize = minDataSize; dataSize <= maxDataSize; dataSize *= 2) {
    ...
    A = (void *) malloc (dataSize+32);
    B = (void *) malloc (dataSize+32);

    clA = clCreateBuffer (ctx, CL_MEM_READ_ONLY, dataSize+32, NULL, &err);
    CL_CHECK_ERROR (err);

    clB = clCreateBuffer (ctx, CL_MEM_READ_WRITE, dataSize+32, NULL, &err);
    CL_CHECK_ERROR (err);

    err = clEnqueueWriteBuffer (queue, clA, CL_TRUE, 0, dataSize+32, A, 0, NULL, NULL);
    CL_CHECK_ERROR (err);

    err = clEnqueueWriteBuffer (queue, clB, CL_TRUE, 0, dataSize+32, B, 0, NULL, NULL);
    CL_CHECK_ERROR (err);
