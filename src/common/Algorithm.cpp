...
Algorithm& Algorithm::startKernelFunctionFullV1 () {
...
Algorithm& Algorithm::startKernelFunctionFullV2 () {
...
Algorithm& Algorithm::startKernelFunctionFullV3 () {
...
Algorithm& Algorithm::startKernelFunctionFullV1FPGA () {
...
Algorithm& Algorithm::generateForsFullV1 (bool onlyMeta) {
  ...
  if (!onlyMeta) {
    ...
    oss << getIndent () << "const int WGStart = XGRid * 256;" << endl;
    ...
    generateSingleForFullV1 (0, indexingFormulas, indexingFormulasPrev, indexingLocalMem);
  }
...
Algorithm& Algorithm::generateForsFullV2 (bool onlyMeta) {
  ...
  if (!onlyMeta) {
    ...
    oss << endl;
	  oss << getIndent () << "__local float A[256];" << endl;
		oss << getIndent () << "A[XLid] = GIn[XGL];" << endl;
    oss << getIndent () << "barrier (CLK_LOCAL_MEM_FENCE);" << endl;
    ...
    generateSingleForFullV2 (0, indexingFormulas, indexingFormulasPrev, indexingLocalMem);
  }
...
Algorithm& Algorithm::generateForsFullV3 (bool onlyMeta) {
  ...
  if (!onlyMeta) {
    ...
    generateSingleForFullV3 (0, indexingFormulas, indexingFormulasPrev, indexingLocalMem);
  }
...
Algorithm& Algorithm::generateForsFullV1FPGA (bool onlyMeta) {
  ...
  if (!onlyMeta) {
    ...
    generateSingleForFullV1FPGA (0, indexingFormulas, indexingFormulasPrev, indexingLocalMem);
  }
...
Algorithm& Algorithm::generateSingleForFullV1 (int loopIndex,
                                           vector<vector<string> >& indexingFormulas,
                                           vector<vector<string> >& indexingFormulasPrev,
                                           vector<vector<string> >& indexingLocalMem) {
  ...
  if (loopIndex == 0) {
    ...
    if (vectorSize == 1) {
      ...
      oss << getIndent () << "float NF = (float) N;" << endl;
			oss << getIndent () << "float PF = (float) P;" << endl;
      oss << getIndent () << "float tempOut = XGL;" << endl;
      oss << getIndent () << "float tempOutTotal = 0;" << endl;
    }
    else {
      //oss << getIndent () << "float MF = (float) XGL;" << endl;
      oss << getIndent () << "float NF = (float) N;" << endl;
  ...
  if (dependency == false) {
    ...
    CircularNumberGenerator CNG (memAllocationPerWorkItem);
    if (operationalIntensity != OperationalIntensity::HALF) {
      int divisible = 0;
      if (operationalIntensity == OperationalIntensity::ONE) {
        divisible = 2;
      } else if (operationalIntensity == OperationalIntensity::TWO) {
        divisible = 4;
      } else if (operationalIntensity == OperationalIntensity::FOUR) {
        divisible = 8;
      } else if (operationalIntensity == OperationalIntensity::EIGHT) {
        divisible = 16;
      } else if (operationalIntensity == OperationalIntensity::SIXTEEN) {
        divisible = 32;
      }

      for (int i = 0; i < numOfInstructions / divisible; i++) {
				string formulaRepl (formula);
      	stringstream replacement;
				if (loopCarriedDepDegree == 1) {
      		replacement << "XGL" << " + " << i << "*XLSize";
      		replace (formulaRepl, "XGL", replacement.str());
        } else {
          replacement << "XGL" << " + " << i << "*256" << " + " << "lcdd*" << 256*(numOfInstructions/divisible);
          replace (formulaRepl, "XGL", replacement.str());
        }
      	//replace (formulaRepl, "XGL", replacement.str());
        ...
        for (int j = 0; j < divisible-1; j++) {
          string formulaRepl (formula);
          replace (formulaRepl, "GIn[XGL]", "tempOut");
          replace (formulaRepl, "tempOutTotal", "temp1");
          oss << getIndent () << formulaRepl << ";" << endl;
        }
      }
    } else {
        ...
        if (loopCarriedDepDegree == 1) {
          ...
          replacement << "XGL" << " + " << i << "*256" << " + " << "lcdd*" << 256*numOfInstructions;
          replace (formulaRepl, "XGL", replacement.str());
        }
      	//replace (formulaRepl, "XGL", replacement.str());
      	if (useLocalMem && algorithmTargetDevice != AlgorithmTargetDevice::FPGA) {
    ...
    oss << getIndent () << "tempOutTotal = temp1 + temp2 + temp3 + temp4;" << endl;
    ...
    // Now it's time to recursively call the single for algorithm,
    // in order to generate the next block of work items.
    generateSingleForFullV1 (loopIndex+1, indexingFormulas, indexingFormulasPrev, indexingLocalMem);
...
Algorithm& Algorithm::generateSingleForFullV2 (int loopIndex,
                                           vector<vector<string> >& indexingFormulas,
                                           vector<vector<string> >& indexingFormulasPrev,
                                           vector<vector<string> >& indexingLocalMem) {
  ...
  if (dependency == false) {
    ...
    if (operationalIntensity != OperationalIntensity::HALF) {
      ...
      for (int i = 0; i < numOfInstructions / divisible; i++) {
        ...
        for (int j = 0; j < divisible-1; j++) {
          string formulaRepl (formula);
          replace (formulaRepl, "A[XLid]", "tempOut");
          replace (formulaRepl, "tempOutTotal", "temp1");
          oss << getIndent () << formulaRepl << ";" << endl;
        }
      }
    ...
    // Now it's time to recursively call the single for algorithm,
    // in order to generate the next block of work items.
    generateSingleForFullV2 (loopIndex+1, indexingFormulas, indexingFormulasPrev, indexingLocalMem);
...
Algorithm& Algorithm::generateSingleForFullV3 (int loopIndex,
                                           vector<vector<string> >& indexingFormulas,
                                           vector<vector<string> >& indexingFormulasPrev,
                                           vector<vector<string> >& indexingLocalMem) {
  ...
  if (dependency == false) {
    ...
    if (operationalIntensity != OperationalIntensity::HALF) {
      ...
      for (int i = 0; i < numOfInstructions / divisible; i++) {
      	...
      	stringstream replacement;
				if (loopCarriedDepDegree == 1) {
      		replacement << "(XGRid+XLid)*" << numOfInstructions/divisible << " + " << i;
      		replace (formulaRepl, "XGL", replacement.str());
        } else {
          replacement << "(XGRid+XLid)*" << numOfInstructions/divisible << " + " << i << " + " << "lcdd*" << 256*(numOfInstructions/divisible);
          replace (formulaRepl, "XGL", replacement.str());
        }
      	//replace (formulaRepl, "XGL", replacement.str());
        ...
        if (loopCarriedDepDegree == 1) {
      		replacement << "(XGRid+XLid)*" << numOfInstructions << " + " << i;
      		replace (formulaRepl, "XGL", replacement.str());
        } else {
          replacement << "(XGRid+XLid)*" << numOfInstructions <<  " + " << i << " + " << "lcdd*" << 256*numOfInstructions;
          replace (formulaRepl, "XGL", replacement.str());
        }
      	//replace (formulaRepl, "XGL", replacement.str());
      	if (useLocalMem && algorithmTargetDevice != AlgorithmTargetDevice::FPGA) {
    ...
    // Now it's time to recursively call the single for algorithm,
    // in order to generate the next block of work items.
    generateSingleForFullV3 (loopIndex+1, indexingFormulas, indexingFormulasPrev, indexingLocalMem);
...
Algorithm& Algorithm::generateSingleForFullV1FPGA (int loopIndex,
                                           vector<vector<string> >& indexingFormulas,
                                           vector<vector<string> >& indexingFormulasPrev,
                                           vector<vector<string> >& indexingLocalMem) {
  ...
  if (dependency == false) {
    ...
    // Now it's time to recursively call the single for algorithm,
    // in order to generate the next block of work items.
    generateSingleForFullV1FPGA (loopIndex+1, indexingFormulas, indexingFormulasPrev, indexingLocalMem);
...
Algorithm& Algorithm::popMetasFullV1 () {
...
Algorithm& Algorithm::popMetasFullV2 () {
...
Algorithm& Algorithm::popMetasFullV3 () {
...
int Algorithm::getOperationalIntensity () {
